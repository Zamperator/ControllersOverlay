...
export default function ArcadeVenom() {
    ...
    useEffect(() => {
        function update() {
            ...
            const gp = Array.from(pads).find(p => p && /Arcade|Venom|Hori|Fight|Stick/i.test(p.id))
            ...
            if (stickBall.current) {
                ...
                if (magnitude > 0.1) {
                    ...
                    stickBall.current.style.transform = `translate(calc(-50% + ${moveX}vmin), calc(-50% + ${moveY}vmin))`

                    // Theme-abhängige Farben (via CSS-Variablen)
                    const light = getComputedStyle(document.documentElement).getPropertyValue("--ball-light").trim() || "#ff4040"
                    const dark = getComputedStyle(document.documentElement).getPropertyValue("--ball-dark").trim() || "#600000"
                    const shadow = getComputedStyle(document.documentElement).getPropertyValue("--ball-shadow-glow").trim() || "rgba(255,0,0,0.5)"

                    const lightX = 50 - moveX * 1.5
                    const lightY = 50 - moveY * 1.5

                    stickBall.current.style.background = `radial-gradient(circle at ${lightX}% ${lightY}%, ${light}, ${dark})`
                    stickBall.current.style.boxShadow = `inset 0 0 1.2vmin var(--ball-shadow-inset, #300), ${-moveX * 0.2}vmin ${-moveY * 0.2}vmin 1.2vmin ${shadow}`
                } else {
                    stickBall.current.style.transform = "translate(-50%, -50%)"
                    stickBall.current.style.background = "radial-gradient(circle at 30% 30%, var(--ball-light, #ff3030), var(--ball-dark, #800000))"
                    stickBall.current.style.boxShadow = "inset 0 0 1vmin var(--ball-shadow-inset, #500), 0 0 1vmin var(--ball-shadow-glow, #f00)"
                }
            }
